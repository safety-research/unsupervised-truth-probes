...
def train_ccs_saliency_probe(
    ...
    weight_decay: float = 0.01,
    k_folds: int = 5,
    saliency_weight: float = 1.0,  # Weight for cross-consistency loss
    ccs_weight: float = 1.0,  # Weight for original CCS loss
    verbose: bool = False,
):
    """
    Train probe with CCS + Cross-Consistency Saliency loss.

    This combines:
    1. Original CCS consistency + confidence losses
    2. Cross-validation consistency where specialist probes trained on subsets
       should agree with the meta probe on held-out data
    """
    assert (
        probe.num_classes == 1
    ), "CCS+Saliency requires binary probe with single output"
    assert X_pos.shape[0] == X_neg.shape[0], "Requires same number of pos/neg examples"
    ...
    # Training loop
    epoch_pbar = tqdm(
        range(num_epochs), desc="Training CCS+Saliency", disable=not verbose
    )
    ...
    for epoch in epoch_pbar:
        ...
        for batch_idx in range(n_batches):
            ...
            # ===== Original CCS Loss =====
            logits_pos = probe(X_pos_batch).squeeze(-1)
            logits_neg = probe(X_neg_batch).squeeze(-1)
            ...
            ccs_loss = consistency_loss + confidence_loss

            # ===== Cross-Consistency Saliency Loss =====
            saliency_loss = _compute_saliency_loss(
                X_pos_batch, X_neg_batch, p_pos, p_neg, k_folds
            )

            # Combined loss
            total_loss = ccs_weight * ccs_loss + saliency_weight * saliency_loss
            ...
            epoch_ccs_loss += ccs_loss.item()
            epoch_saliency_loss += saliency_loss.item()

        # Update progress bar
        if verbose:
            avg_loss = epoch_loss / n_batches
            avg_ccs = epoch_ccs_loss / n_batches
            avg_saliency = epoch_saliency_loss / n_batches

            epoch_pbar.set_postfix(
                {
                    "total": f"{avg_loss:.6f}",
                    "ccs": f"{avg_ccs:.6f}",
                    "saliency": f"{avg_saliency:.6f}",
                }
            )
...
def _compute_saliency_loss(
    X_pos: torch.Tensor,
    X_neg: torch.Tensor,
    main_p_pos: torch.Tensor,
    main_p_neg: torch.Tensor,
    k_folds: int = 5,
    specialist_probe_kwargs: dict = {},
):
    """
    Cross-consistency saliency loss: train specialist probes on main probe's
    predictions, then measure how well they generalize to held-out data.
    """
    if len(X_pos) < k_folds:
        return torch.tensor(0.0, device=X_pos.device, requires_grad=True)

    # Get k-fold splits
    fold_indices, train_indices = create_k_fold_splits(len(X_pos), k_folds)

    # Prepare training data for specialists (detached to prevent gradient flow)
    X_train_all = torch.cat(
        [X_pos[train_indices], X_neg[train_indices]], dim=0
    ).detach()
    y_train_all = (
        torch.cat([main_p_pos[train_indices], main_p_neg[train_indices]], dim=0)
        .detach()
        .unsqueeze(-1)
    )

    # Train k specialist probes simultaneously
    specialist_probes = MultiLinearProbe(X_train_all.shape[-1], 1, k_folds).to(
        X_pos.device
    )
    specialist_probes = train_supervised_probe(
        probe=specialist_probes,
        X=X_train_all,
        y=y_train_all,
    ...
    # Prepare test data
    X_test_all = torch.cat([X_pos[fold_indices], X_neg[fold_indices]], dim=0).detach()
    main_predictions_test = torch.cat(
        [main_p_pos[fold_indices], main_p_neg[fold_indices]], dim=0
    )

    # Get specialist predictions on test data
    with torch.no_grad():
        specialist_logits = specialist_probes(X_test_all)
        specialist_log_probs = F.logsigmoid(specialist_logits.squeeze(-1))

    # KL divergence: how well specialists match main probe on held-out data
    kl_loss = F.kl_div(
        specialist_log_probs, main_predictions_test, reduction="batchmean"
    )
